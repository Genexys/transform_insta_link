...
function registerCommandHandlers(bot) {
    ...
    async function getBotUsername() {
        if (!botUsernamePromise) {
            botUsernamePromise = bot
                ...
                .catch(err => {
                runtime_1.log.error('Failed to resolve bot username in command handlers', {
                    err: String(err),
                });
                return null;
            });
    ...
    async function getBotStartGroupUrl() {
        const username = await getBotUsername();
        return username ? `https://t.me/${username}?startgroup=true` : null;
    }
    async function getBotReferralUrl(telegramId) {
        const username = await getBotUsername();
    ...
    async function getOnboardingText() {
        const botMention = await getBotMention();
        return ('👋 Привет! Я автоматически исправляю ссылки соцсетей, чтобы они показывали превью прямо в чате.\n\n' +
            'Поддерживаю: Instagram, TikTok, Twitter/X, Reddit, Bluesky, Pixiv, DeviantArt\n\n' +
    ...
    bot.onText(/^\/start(?:@\w+)?(?:\s+(.+))?$/, async (msg) => {
        ...
        if (telegramId && param?.startsWith('ref_')) {
            ...
            if (!isNaN(referrerId) && referrerId !== telegramId) {
                ...
                await (0, db_1.setReferredBy)(telegramId, referrerId);
            }
        }
    ...
    bot.onText(/^\/invite(?:@\w+)?(?:\s|$)/, async (msg) => {
        ...
        if (!telegramId)
            return;
        await (0, db_1.createUser)(telegramId, msg.from?.username);
        const count = await (0, db_1.getReferralCount)(telegramId);
        ...
        const opts = {
            ...
            reply_markup: {
                inline_keyboard: [
                    [
                        {
                            text: `⭐ ${billing_1.DONATE_AMOUNTS_STARS[0]} Stars`,
                            callback_data: `donate_${billing_1.DONATE_AMOUNTS_STARS[0]}`,
                        },
                        {
                            text: `⭐ ${billing_1.DONATE_AMOUNTS_STARS[1]} Stars`,
                            callback_data: `donate_${billing_1.DONATE_AMOUNTS_STARS[1]}`,
                        },
                    ],
                    [
                        {
                            text: `⭐ ${billing_1.DONATE_AMOUNTS_STARS[2]} Stars`,
                            callback_data: `donate_${billing_1.DONATE_AMOUNTS_STARS[2]}`,
                        },
                        {
                            text: `⭐ ${billing_1.DONATE_AMOUNTS_STARS[3]} Stars`,
                            callback_data: `donate_${billing_1.DONATE_AMOUNTS_STARS[3]}`,
                        },
                    ],
                ],
            },
    ...
    bot.onText(/^\/chatstats(?:@\w+)?(?:\s|$)/, async (msg) => {
        ...
        let userRes;
        try {
            platformRes = await db_1.dbClient.query(`SELECT platform, COUNT(*) as cnt
         FROM link_events
         WHERE chat_id = $1
           AND created_at >= NOW() - INTERVAL '7 days'
         GROUP BY platform
         ORDER BY cnt DESC`, [chatId]);
            userRes = await db_1.dbClient.query(`SELECT user_id, COUNT(*) as cnt
         FROM link_events
         WHERE chat_id = $1
         ...
         LIMIT 3`, [chatId]);
            ...
            runtime_1.log.error('chatstats query failed', { chatId, err: String(err) });
            await bot.sendMessage(chatId, '📊 Не удалось загрузить статистику чата. Попробуйте позже.');
            return;
        }
    ...
    bot.on('my_chat_member', async (update) => {
        ...
        const justAdded = (new_chat_member.status === 'member' ||
        ...
        try {
            ...
            runtime_1.log.info('Onboarding message sent', {
                chatId: chat.id,
                chatTitle: chat.title,
            });
        }
        catch (err) {
            runtime_1.log.error('Failed to send onboarding message', {
                chatId: chat.id,
                err: String(err),
            });
        }
